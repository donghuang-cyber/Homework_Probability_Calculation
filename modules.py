...
graph = Graph("http://127.0.0.1:7474", auth=("neo4j", "neo4j_hxd"))
...
def semantic_parser(text, user):
    ...
    if intent_rst == -1 or slot_rst == -1 or intent_rst.get("intent") == "其他":
        return semantic_slot.get("unrecognized")

    slot_info = semantic_slot.get(intent_rst.get("intent"))
   # print('*******************************')
   # print(slot_info)
    # 填槽
